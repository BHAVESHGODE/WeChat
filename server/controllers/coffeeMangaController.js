...
const BASE = 'https://coffeemanga.io';
...
function extractMangaList(html) {
  const items = [];
  const regex = /<div[^>]*class="[^"]*bs[^"]*"[^>]*>([\s\S]*?)<\/div>\s*<\/div>\s*<\/div>/gi;
  let m;
  while ((m = regex.exec(html)) !== null) {
    ...
    const rating = (block.match(/rating[^\d]*(\d+(?:\.\d+)?)/i) || [])[1];
    if (title) items.push({ title: title.trim(), imageUrl: img || null, url: link || '', latestChapter: chapter ? parseFloat(chapter) : null, rating: rating ? parseFloat(rating) : null, source: 'coffeemanga' });
  }
  return items;
}
...
function extractDetail(html) {
  ...
  const img = (html.match(/<img[^>]*class="[^"]*wp-manga-cover[^"]*"[^>]*src="([^"]+)"/i) || [])[1] || '';
  ...
  const genres = [];
  const gRegex = /<a[^>]*href="[^"]*\/(genre|genres)\/[^"]+"[^>]*>([^<]+)<\/a>/gi;
  let g;
  while ((g = gRegex.exec(html)) !== null) genres.push(g[2]?.trim() || g[1]);
  return { title, description: desc, imageUrl: img, author, status, year, genres, source: 'coffeemanga' };
}
...
async function searchManga(req, res) {
  try {
    ...
    res.json({ data: mangaList, page: parseInt(page), source: 'coffeemanga' });
  } catch (err) {
    res.status(500).json({ error: 'CoffeeManga search failed', detail: err.message });
  }
}
...
async function getMangaList(req, res) {
  try {
    ...
    res.status(500).json({ error: 'CoffeeManga list failed', detail: err.message });
  }
}
...
async function getMangaDetail(req, res) {
  try {
    ...
    res.status(500).json({ error: 'CoffeeManga detail failed', detail: err.message });
  }
}
...
async function getChapterPages(req, res) {
  try {
    ...
    res.status(500).json({ error: 'CoffeeManga chapter failed', detail: err.message });
  }
}
